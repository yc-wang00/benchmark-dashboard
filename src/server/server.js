...
const { getBenchmarkData, getBenchmarkDataALL, createBenchmarkData } = require('./db.js');
...
app.post('/benchmarks', async (req, res) => {
  const { data_path, takeoff_image, model_name, device, max_batch_size, max_seq_len, backend, expected_duration, expected_vus, expected_iterations, git_commit_hash, num_gpu, gpu_name, gpu_memory, gpu_memory_unit, num_passes, num_fails, run_duration_sec, run_vus, run_iterations, avg_req_duration_sec, max_req_duration_sec, min_req_duration_sec } = req.body;
  const benchmark = await createBenchmarkData(data_path, takeoff_image, model_name, device, max_batch_size, max_seq_len, backend, expected_duration, expected_vus, expected_iterations, git_commit_hash, num_gpu, gpu_name, gpu_memory, gpu_memory_unit, num_passes, num_fails, run_duration_sec, run_vus, run_iterations, avg_req_duration_sec, max_req_duration_sec, min_req_duration_sec);
